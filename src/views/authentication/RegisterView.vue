...
<template>
    ...
    <div class="container">
        ...
        <v-row dense class="form-row">
            <v-col cols="12" lg="6">
                <VTextField v-model="form.firstName" required :placeholder="t('auth.register.first')"
                            hide-details="auto"
            ...
            <v-col cols="12" lg="6">
                <VTextField v-model="form.username" required :placeholder="t('auth.register.username')"
                            hide-details="auto"
                            class="custom-input"></VTextField> <!-- New Username Field -->
            </v-col>
            <v-col cols="12" lg="6">
                <VTextField v-model="form.email" required type="email" :placeholder="t('auth.register.email')"
                            hide-details="auto"
                            class="custom-input"></VTextField>
            </v-col>
            <v-col cols="12" lg="6">
                <VTextField v-model="form.phone" required type="text" :placeholder="t('auth.register.phone')"
                            hide-details="auto"
                            class="custom-input"></VTextField>
            </v-col>
            <v-col cols="12" lg="6">
                <VTextField v-model="form.country" required :placeholder="t('auth.register.country')"
                            hide-details="auto"
                            class="custom-input"></VTextField>
            </v-col>

            <v-col cols="12" lg="6">
                <VTextField v-model="form.password" required :placeholder="t('auth.register.password')"
            ...
            <v-col cols="12" lg="6">
                <VTextField v-model="form.password_confirmation" required :placeholder="t('auth.register.confirm')"
                            :type="show_password ? 'text' : 'password'"
                            @click:append-inner="show_password = !show_password" hide-details="auto"
                            :append-inner-icon="show_password ? 'mdi-eye' : 'mdi-eye-off'"
                            class="custom-input"></VTextField>
        ...
        <v-row>
            <v-col cols="12" class="pr-2">
                <v-btn variant="outlined" size="large" class="border text-subtitle-1" block>
                    ...
                    <span class="d-sm-flex mr-1">Sign up with</span>Google
                </v-btn>
            </v-col>
        </v-row>
        <!-- Checkbox to agree with terms and conditions -->
        <div class="d-flex  mt-2" justify="start">
            <v-checkbox class=" text-primary " :label="t('auth.register.checkboxText')" readonly v-model="checkbox"
                        :indeterminate="false"
                        @click="openTermsDialog"></v-checkbox>
        </div>
        ...
        <v-dialog v-model="terms_dialog" persistent max-width="800">
            <v-card class="overflow-y-auto">
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn variant="text" @click="() => { agree = false; terms_dialog = false }">
                        {{ t('auth.register.disagree') }}
                    </v-btn>
                    <v-btn color="primary" variant="flat" @click="() => { agree = true; terms_dialog = false }">
